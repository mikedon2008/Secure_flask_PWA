...
# Route for handling sign-up form and adding user to database
...
def add_user():
    ...
    if len(ans) > 0:
        ...
        cursor.execute("INSERT INTO USERS(fname, lname, email, password) VALUES (?, ?, ?, ?)", (fname, lname, email, password))
        connection.commit()
        connection.close()
        return render_template('login.html')
